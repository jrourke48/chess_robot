    ...
    try:
        ...
        while True:
            ...
            if ids is not None:
                ...
                print(f"Frame {frame_count}: Detected markers: {ids.flatten().tolist()}")
            else:
                print(f"Frame {frame_count}: No markers detected")

            warped = warp_board(img, found)

            # Save images every 30 frames (about 1 per second)
            if frame_count % 30 == 0:
                cv2.imwrite(f"captured_frame_{frame_count}.jpg", img)
                if warped is not None:
                    cv2.imwrite(f"warped_board_{frame_count}.jpg", warped)
                    print(f"  -> Saved warped board image")
